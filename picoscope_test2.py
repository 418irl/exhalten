#if you change amplitude, change configure_channels(chA_range=7), value of range depends on amplitude
#if you reduce amp, change trigger_adc = 1638
#if you change frequency, change fs = 1e6
#change duration from 0.001
...
def configure_channels(chA_range=4): # 2V range 
# Set up channel A
...
def compute_fft(voltage, fs):
    ...
    fft_dbv = 20*np.log10(np.maximum(magnitude, 1e-20))
...
#def plot_waveform(time_ms, voltage, actual_fs, duration):
    ...
    plt.plot(time_ms[:200], voltage[:200])
    plt.grid(True)
    plt.show()'''
...
def main():
   ...
   # timebase = 127
    ...
    filename=r"C:\Users\fores\OneDrive\Desktop\xalten\xalten\picoscope_csv\test2.csv"
